...
v2.7 移除了旧版注入文本中的显式元说明，改为仅使用自然的 User / Assistant 连续对话格式，并把当前用户输入作为最后一条 User 内容接在末尾。

需要注意：DeepSeek 网页端当前只提交当前 prompt，真实历史仍由服务器根据会话与父消息标识读取，因此客户端无法把导入内容变成真正的服务端历史节点。
